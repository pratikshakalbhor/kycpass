...
with test MID from the Preview faucet, then click **"Deploy contract"** in the
frontend and write the returned address into `.env` as `VITE_CONTRACT_ADDRESS`.
...
- **Frontend:** Vite + React 18 + TypeScript + Tailwind 4, DApp Connector for
  wallet auth, Midnight indexer for the deployed contract state.
...
npm run test                          # compiles first, then vitest run
...
tests/       KYCCredential.test.ts      (7 specs incl. the privacy audit)
frontend/    Vite + React + Tailwind (DApp Connector + indexer)
